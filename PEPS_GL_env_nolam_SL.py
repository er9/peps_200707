...
def get_next_boundary_O(peps_u_row,peps_d_row,boundary,XMAX=100,scaleX=1,DS_fn=DL_to_SL_bound):
    ...
    boundary_mpo  = MPX.canonicalize(boundary_mpo,0)
    boundary_mpo, err2 = MPX.compress_reg(boundary_mpo,XMAX,1,tol=None)
...
def get_next_boundary_I(peps_u_row,peps_d_row,boundary,XMAX=100,scaleX=1,DS_fn=DL_to_SL_bound):
    ...
    boundary_mpo, err1 = MPX.compress_reg(boundary_mpo,XMAX,0)
    ...
    boundary_mpo  = MPX.canonicalize(boundary_mpo,1)
    boundary_mpo, err2 = MPX.compress_reg(boundary_mpo,XMAX,0)
...
def get_next_subboundary_O(env_mpo,boundL_tens,peps_u_row,peps_d_row,boundR_tens,XMAX=100,c_tol=1.0e-8,
    ...
    e_mpo, err1 = MPX.compress_reg(e_mpo,XMAX,0,tol=c_tol)
    ...
    e_mpo  = MPX.canonicalize(e_mpo,1)
    e_mpo, err2 = MPX.compress_reg(e_mpo,XMAX,0,tol=c_tol)
...
def get_next_subboundary_I(env_mpo,boundL_tens,peps_u_row,peps_d_row,boundR_tens,XMAX=100,c_tol=1.0e-8,
    ...
    e_mpo, err1 = MPX.compress_reg(e_mpo,XMAX,1,tol=c_tol)
    ...
    e_mpo  = MPX.canonicalize(e_mpo,0)
    e_mpo, err2 = MPX.compress_reg(e_mpo,XMAX,1,tol=c_tol)
...
def get_subboundaries(bound1,bound2,peps_u_sub,peps_d_sub,side,upto,init_sb=None,XMAX=100,c_tol=1.0e-8,scaleX=1,
                      get_errs=False,DS_fn=DL_to_SL_bound):
...
def contract_2_bounds(bound1, bound2, bonds_u, bonds_d, orientation):
    ...
    if orientation == 'col':
        for i in range(0,len(bound1),2):
            b2.append( np.einsum('lur,uU->lUr',bound2[i]  ,np.diag(bonds_u[int(i/2)])) )
            b2.append( np.einsum('ldr,dD->lDr',bound2[i+1],np.diag(bonds_d[int(i/2)])) )
    elif orientation == 'row':
        for i in range(0,len(bound1),2):
            b2.append( np.einsum('lur,uU->lUr',bound2[i]  ,np.diag(bonds_d[int(i/2)])) )
            b2.append( np.einsum('ldr,dD->lDr',bound2[i+1],np.diag(bonds_u[int(i/2)])) )
