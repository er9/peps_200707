...
if True:
    ...
    for DMAX in Ds:
        ...
        for hx in [0.]: #,0.05,0.25]:
            ...
            # ### start from pure state ###
            # print 'tldm pure init state'
            # try:
            #     ## load SU ground state
            #     su_str = 'L%dx%d_D%d_Xo%d_dt%s_hx%s_hz%s_J2%s'%(L1,L2,DMAX,XMAX2,dt_str,hx_str,hz_str,J2_str)
            ...
            #     init_peps = PEPX_GL.PEPX_GL(gammas,lambdas)
            #     initDM = TLDM_GL.pure_state(init_peps)
            #     print 'using saved SU state as initial state'

            #     print 'init peps nrg', PEPX_GL.meas_obs(init_peps, obs_pepos['H'], XMAX=100)/L1/L2

            # except(IOError):
            #     initDM = TLDM_GL.random_pure_product_state([[(2,)]*L2]*L1)
            ...
            print 'init tldm nrg', run_data_FU['H'][0]/L1/L2
                    ...
                    while not run_success:
                        ...
                        try:
                            obs_t2a, tldm2 = TE_DM.run_TE(dt,d_beta,trotterH,tldm2,DMAX=DMAX,XMAX=XMAX,XMAX2=XMAX2,
                                                          ...
                                                          qr_reduce=qr_reduce,contract_SL=do_SL)
