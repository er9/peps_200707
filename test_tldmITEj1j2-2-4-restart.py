...
restart = True
restart_beta = 0.4  # beta/2 * 2
target_betas = [0.1,0.2,0.5,1.0,1.5,2.0,3.0,5.0,7.5] #10.0,15.0]:  # beta/2

Ls = (5,5)  #(3,3)
L1,L2 = Ls
print 'lattice size', Ls

fdir = 'j1j2_tldm_data/' #DL_data/'
subdir = ''
...
for DMAX in Ds:
  ...
  for J2 in [0.9]:  #[0.3,0.35,0.4]: #,0.05,0.25]:
    ...
    if restart:
        ...
        print('restart target betas',target_betas)
        beta_str_0 = '%04d'%(int(restart_beta*100))
    ...
    # for d_beta in np.abs(np.diff([0.0, 0.1,0.25,0.5,0.75,1.0,1.5,2.0,3.0,5.0,10.0,15.0])):
        ...
        print('target beta', target_beta, betas[-1]/2, d_beta)
        nsteps = int(round(abs(d_beta/dt),2))
        print(nsteps,d_beta,dt)
        ...
        if do_FU:
            ...
            if save_data:
                ...
                try:
                    ...
                    print('errorm saving separate file')
                    np.savetxt(fdir+subdir+file_str+'_FU_X%d_data_restart.txt'%(XMAX),np.real(np.array(FU_data)).T,header=header_str)
