...
def get_corrs(fdir,fstr,corr_type,do_mp=True,is_tldm=False,subdir=None,save_subdir=None):
    ...
    if corr_type in ['SS','spin']:
        try:
            spin_corrs  = load_obj(fdir+save_subdir,fstr+'_SScorrs')

        except(IOError):
            print 'no corrs: '+fdir+save_subdir+fstr+'_SScorrs.pkl'
            try:
        ...
        try:
            dimer_corrs = load_obj(fdir+save_subdir,fstr+'_DDcorrs')
        except(IOError):
            try:
                gammas  = np.load(fdir+subdir+fstr+'_gammas.npy' , allow_pickle=True)
                lambdas = np.load(fdir+subdir+fstr+'_lambdas.npy', allow_pickle=True)
...
J2s_FU = np.linspace(0.3,1.0,15)
J2s_FU = np.linspace(0.4,1.0,7)
J2s_FU = [0.8,0.9,1.0]
...
if is_FU:
    ...
    for DMAX in Ds:
        ...
        for J2 in J2s:
            ...
            J2str = '%04d'%int(100*round(J2,2))
            print('J2str',J2str,J2)
            if is_tldm:
            ...
            for fnum in range(len(fn_gs)):
                ...
                try:
                    ...
                    #### spin corrs ####
                    if is_tldm:   spin_corrs = get_corrs(fdir,fstr,'SS',do_mp=do_mp,is_tldm=True,subdir='tldm_states/')
                    else:         spin_corrs = get_corrs(fdir,fstr,'SS',do_mp=do_mp)
                    ...
                    #### dimer corrs ####
                    if do_dimer:
                        if is_tldm:    dimer_corrs = get_corrs(fdir,fstr,'DD',do_mp=do_mp,is_tldm=True,subdir='tldm_states')
                        else:          dimer_corrs = get_corrs(fdir,fstr,'DD',do_mp=do_mp)
