...
restart = True
# restart_beta = 12.0  # beta/2 * 2
# target_betas = [6.5,7.0,7.5] #10.0,15.0]  # beta/2
restart_beta = 14.0  # beta/2 * 2
...
for DMAX in Ds:
  ...
  for J2 in [0.8]: # [0.6]:  #[0.3,0.35,0.4]: #,0.05,0.25]:
  # for J2 in [0.6]:
    ...
    # for d_beta in np.abs(np.diff([0.0, 0.1,0.25,0.5,0.75,1.0,1.5,2.0,3.0,5.0,10.0,15.0])):
        ...
        if do_FU:
            ...
            if save_data:
                ...
                try:
                    # orig_data = np.loadtxt(fdir+subdir+file_str+'_FU_X%d_data_restart.txt'%(XMAX))
                    orig_data = np.loadtxt(fdir+subdir+file_str+'_FU_X%d_data.txt'%(XMAX))
                    orig_data = orig_data.T
                    ind_cut = int(betas[0]/2/np.abs(dt))
                    print('orig data', orig_data.shape, ind_cut, orig_data[0,ind_cut], betas[0])
